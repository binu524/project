...
// 2. 새로운 위치가 유효한지 검사하는 함수 [cite: 34, 35, 36]
const isValidPosition = (movingItem, allItems, roomBoundary) => {
  // A. 방 경계를 벗어나는지 확인 (벽 충돌 방지) [cite: 35, 96]
  if (
    movingItem.x < roomBoundary.x ||
    movingItem.y < roomBoundary.y ||
    movingItem.x + movingItem.width > roomBoundary.x + roomBoundary.width ||
    movingItem.y + movingItem.height > roomBoundary.y + roomBoundary.height
  ) {
    return false;
  }

  // B. 다른 가구와 겹치는지 확인 [cite: 36, 105]
  for (let item of allItems) {
    if (item.id === movingItem.id) continue; // 자기 자신은 제외
    if (checkCollision(movingItem, item)) {
      return false;
    }
  }
