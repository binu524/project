...
const InteriorPlanner = () => {
  ...
  const isDeskItem = (item) => /책상/.test(item.name);
  const isChairItem = (item) => /의자/.test(item.name);
  const getItemRoomId = (item) => getRoomForFurniture(item)?.id;

  const buildDeskChairGroup = (desk, chairs, groupId) => {
    const gap = 10;
    const deskItem = { ...desk, groupId };
    const chairItems = [];

    if (deskItem.width >= deskItem.height) {
      let offsetX = 0;
      chairs.forEach(chair => {
        chairItems.push({ ...chair, groupId, offsetX, offsetY: deskItem.height + gap });
        offsetX += chair.width + gap;
      });
    } else {
      let offsetY = 0;
      chairs.forEach(chair => {
        chairItems.push({ ...chair, groupId, offsetX: deskItem.width + gap, offsetY });
        offsetY += chair.height + gap;
      });
    }

    const maxX = Math.max(deskItem.width, ...chairItems.map(c => c.offsetX + c.width));
    const maxY = Math.max(deskItem.height, ...chairItems.map(c => c.offsetY + c.height));

    return {
      isGroup: true,
      id: `desk-chair-${groupId}`,
      name: '책상 + 의자 세트',
      x: 0,
      y: 0,
      width: maxX,
      height: maxY,
      items: [deskItem, ...chairItems].map(item => ({
        ...item,
  ...
  const generateAiRecommendations = () => {
    ...
    const generateLayout = (type) => {
      ...
      const unlockedItems = placedFurniture.filter(f => !f.isLocked);
      const ungroupedForAuto = unlockedItems.filter(f => !f.groupId);
      const groupedDeskIds = new Set();
      const groupedChairIds = new Set();
      const deskChairGroups = [];

      ungroupedForAuto.filter(isDeskItem).forEach((desk, index) => {
        const candidates = ungroupedForAuto.filter(item => isChairItem(item) && !groupedChairIds.has(item.id));
        if (candidates.length === 0) return;

        const deskRoomId = getItemRoomId(desk);
        const sameRoomCandidates = candidates.filter(chair => getItemRoomId(chair) === deskRoomId);
        const orderedChairs = (sameRoomCandidates.length > 0 ? sameRoomCandidates : candidates)
          .sort((a, b) => {
            const da = Math.hypot(desk.x - a.x, desk.y - a.y);
            const db = Math.hypot(desk.x - b.x, desk.y - b.y);
            return da - db;
          })
          .slice(0, 2);

        if (orderedChairs.length > 0) {
          orderedChairs.forEach(chair => groupedChairIds.add(chair.id));
          groupedDeskIds.add(desk.id);
          deskChairGroups.push(buildDeskChairGroup(desk, orderedChairs, `${Date.now()}-${index}`));
        }
      });

      placedFurniture.filter(f => !f.isLocked && f.groupId).forEach(f => {
      ...
      deskChairGroups.forEach(group => {
        layoutItems.push(group);
        group.items.forEach(subItem => processed.add(subItem.id));
      });

      unlockedItems.filter(f => !processed.has(f.id) && !groupedDeskIds.has(f.id) && !groupedChairIds.has(f.id)).forEach(f => {
        processed.add(f.id);
  ...
  return (
    <div style={{ padding: '40px', backgroundColor: '#fcfaff', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', marginBottom: '40px' }}>
        ...
        <button onClick={() => setActiveTab('recommend')} style={{ padding: '12px 30px', borderRadius: '99px', fontWeight: 'bold', border: 'none', cursor: 'pointer', backgroundColor: activeTab === 'recommend' ? '#ec4899' : '#f3f4f6', color: activeTab === 'recommend' ? '#fff' : '#9ca3af' }}>🛍️ 추천 쇼핑</button>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '40px' }}>
        ...
        <div style={{ width: '340px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {activeTab === 'editor' ? (
            ...
            <div style={{ padding: '24px', backgroundColor: '#fff', borderRadius: '24px', border: '2px solid #ec4899' }}>
              <h3 style={{ fontWeight: '900', fontSize: '20px', marginBottom: '24px', color: '#ec4899' }}>🛍️ 인테리어 추천 쇼핑몰</h3>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <a href="https://ohou.se" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <button style={{ width: '100%', padding: '16px', backgroundColor: '#fef2f2', border: '2px solid #fca5a5', borderRadius: '12px', fontWeight: '900', cursor: 'pointer', color: '#c2410c', fontSize: '15px', transition: 'all 0.3s' }}>
                    🏠 오늘의집
                  </button>
                </a>
                <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 12px 0' }}>국내 최대 규모의 인테리어 커뮤니티 & 쇼핑몰. 실제 사례, 시공 비용, 전문가 상담 가능</p>

                <a href="https://www.29cm.co.kr/content/category-collection/2026/05/monthly-home" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <button style={{ width: '100%', padding: '16px', backgroundColor: '#f0fdf4', border: '2px solid #86efac', borderRadius: '12px', fontWeight: '900', cursor: 'pointer', color: '#166534', fontSize: '15px', transition: 'all 0.3s' }}>
                    🎨 29CM
                  </button>
                </a>
                <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 12px 0' }}>디자인 가구 & 홈데코 전문. 트렌디한 디자이너 브랜드 제품 다수 보유</p>

                <a href="https://www.nordicnest.kr" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <button style={{ width: '100%', padding: '16px', backgroundColor: '#fef3c7', border: '2px solid #fcd34d', borderRadius: '12px', fontWeight: '900', cursor: 'pointer', color: '#92400e', fontSize: '15px', transition: 'all 0.3s' }}>
                    🪴 노르딕네스트
                  </button>
                </a>
                <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 12px 0' }}>북유럽 스타일 가구 & 생활용품. 미니멀하고 따뜻한 감성의 제품</p>

                <a href="https://www.ikea.com/kr/ko/" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <button style={{ width: '100%', padding: '16px', backgroundColor: '#e0f2fe', border: '2px solid #93c5fd', borderRadius: '12px', fontWeight: '900', cursor: 'pointer', color: '#0369a1', fontSize: '15px', transition: 'all 0.3s' }}>
                    🪑 IKEA
                  </button>
                </a>
                <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 12px 0' }}>글로벌 가구 브랜드. 합리적인 가격과 DIY 조립 가구를 쉽게 찾을 수 있어요</p>

                <hr style={{ border: 'none', borderTop: '1px solid #e5e7eb', margin: '16px 0' }} />

                <div style={{ backgroundColor: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '12px', padding: '14px', marginTop: '12px' }}>
                  <p style={{ fontSize: '12px', fontWeight: 'bold', color: '#1e293b', margin: '0 0 8px 0' }}>💡 쇼핑 팁</p>
                  <ul style={{ fontSize: '11px', color: '#475569', margin: '0', paddingLeft: '16px', lineHeight: '1.6' }}>
                    <li>위 AI 배치 추천으로 필요한 가구 크기를 파악한 후 쇼핑하기</li>
                    <li>인테리어 스타일(북유럽, 미니멀, 클래식 등)에 맞춰 선택</li>
                    <li>배송비, AS, 반품 정책 꼼꼼히 확인하기</li>
                    <li>실제 크기와 공간을 고려한 구매 결정</li>
                  </ul>
                </div>
              </div>
            </div>
